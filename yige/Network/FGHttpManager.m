...
//        [manager.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
        ...
        manager.responseSerializer.acceptableStatusCodes = [NSIndexSet indexSetWithIndexesInRange:NSMakeRange(200, 301)];
        // 设置接收的Content-Type
        manager.responseSerializer.acceptableContentTypes=[NSSet setWithObjects:@"application/json", @"text/json",@"text/html",@"text/plain",@"text/xml",@"text/javascript", nil];
...
//        NSLog(@"responseObject-->%@",responseObject);
        ...
        if (obj.code.integerValue == 0) {
            ...
            failure([self handleFailureResponse:obj]);
        }
             ...
             failure:(void (^)(NSString *error))failure
{
    ...
    [[self manager] POST:[BaseApi stringByAppendingPathComponent:path] parameters:parameters progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        //保存token (用户注册时需要用到)
        NSString *token = objectForKey(obj.meta, @"access_token");
        if (!IsEmpty(token)) {
...
+ (void)patchWithPath:(NSString *)path
           parameters:(NSDictionary *)parameters
              success:(void (^)(id responseObject))success
              failure:(void (^)(NSString *error))failure{
    [[self manager] PATCH:[BaseApi stringByAppendingPathComponent:path] parameters:parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        //请求成功返回数据 根据responseSerializer 返回不同的数据格式
        NSLog(@"responseObject-->%@",responseObject);
        FGResponseModel *obj = [FGResponseModel modelWithJSON:responseObject];
...
+ (void)deleteWithPath:(NSString *)path
            parameters:(NSDictionary *)parameters
               ...
               failure:(void (^)(NSString *error))failure
{
    [[self manager] DELETE:[BaseApi stringByAppendingPathComponent:path] parameters:parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        //请求成功返回数据 根据responseSerializer 返回不同的数据格式
        NSLog(@"responseObject-->%@",responseObject);
        FGResponseModel *obj = [FGResponseModel modelWithJSON:responseObject];
...
+ (void)handleStatusCode:(NSURLSessionDataTask *)task
{
    ...
    {
        //重新登录
        [FGUserHelper reloadTokenWithSuccess:nil];
...
+ (NSString *)handleFailureResponse:(FGResponseModel *)response
{
    if (response.code.integerValue == 401 && [response.msg isEqualToString:@"The token has been blacklisted"]) {
        [FGUserHelper logout];
        return @"请重新登录";
    }else if (response.code.integerValue == 401 && [response.msg isEqualToString:@"Token has expired"]) {
        [FGUserHelper logout];
        return @"请重新登录";
    }else{
        return response.msg;
